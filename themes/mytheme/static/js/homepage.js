...
function dIC(x, y, a, ts) {
  ...
  cx.beginPath();cx.moveTo(-sz+2,-sz*.6);cx.lineTo(sz-2,-sz*.6);cx.quadraticCurveTo(sz,-sz*.6,sz,-sz*.6+2);cx.lineTo(sz,sz*.6-2);cx.quadraticCurveTo(sz,sz*.6,sz-2,sz*.6);cx.lineTo(-sz+2,sz*.6);cx.quadraticCurveTo(-sz,sz*.6,-sz,sz*.6-2);cx.lineTo(-sz,-sz*.6+2);cx.quadraticCurveTo(-sz,-sz*.6,-sz+2,-sz*.6);cx.closePath();cx.fill();cx.stroke();
  for (var p = -1; p <= 1; p++) {
...
var lsp = 0;
function frame(ts) {
  if(!cx){requestAnimationFrame(frame);return;}
  try{
...
function startCanvas(){cv=document.getElementById("cv");if(!cv){setTimeout(startCanvas,50);return;}rsz();requestAnimationFrame(frame);}
if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",startCanvas);}else{startCanvas();}
